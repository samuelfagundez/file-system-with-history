...
export class Node {
	...
	constructor(name: string, type: NodeType, content?: string, parent?: Node) {
		this.childs = {};
		...
		if (!name)
			throw new Error('El nombre de un directorio no puede ser vacío.');
		this.name = name;

		if (type !== 'f' && content)
			throw new Error('Un directorio solo puede tener contenido.');
	...
	public create(nombre: string, type: NodeType): void {
		...
		if (!nombre)
			throw new Error('El nombre del directorio no puede ser vacío.');
	...
	public read(nombre: string): string {
		...
		if (this.childs[nombre].type !== 'f')
			throw new Error('El archivo a leer no es un archivo.');
	...
	public escribir(nombre: string, content: string): void {
		if (!nombre)
			throw new Error(
				'El nombre del archivo a escribir no puede ser vacío.'
			);
		...
		if (this.childs[nombre].type !== 'f')
			throw new Error('El archivo a escribir no es un archivo.');
