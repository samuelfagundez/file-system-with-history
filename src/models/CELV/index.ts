...
export class CELV {
  ...
  public areNewChanges(): boolean {
    if (!this.is_initialized) throw new Error("CELV no está inicializado.");
    if (!this.root_node) throw new Error("No existe un nodo raíz.");
    if (!this.current_version) throw new Error("No existe una versión actual.");

    const previous_root = this.version_directory[this.version_history[this.version_history.length - 1]].root;
  ...
  public celv_vamos(versionId: string): void {
    if (!this.is_initialized) throw new Error("CELV no está inicializado.");
    if (!this.root_node) throw new Error("No existe un nodo raíz.");
    if (!this.version_history.includes(versionId)) throw new Error("No existe la versión deseada.");
